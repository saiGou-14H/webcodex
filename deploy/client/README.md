...
| `add-mcp` | **自动生成 `wc_pat_xxx`**：调 `webcodex tokens create-local` 向服务器注册并取回明文 token，缓存 `mcp.bearer`，并**直接写入 Codex 配置** `[mcp_servers.kunkun-tools]` + `http_headers.Authorization = "Bearer <wc_pat>"`（不依赖环境变量） |
...
- **mcp 模式**：本地 Codex 直连 `<server>/mcp`（`config.toml` 已内嵌 `Authorization: Bearer <wc_pat>`，不依赖环境变量）。
